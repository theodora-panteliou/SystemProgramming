...
/* inserts a record containing {id, first name, last name, country, age, [date]} with given key, virus, answer to data structures and returns the pointer to the Vaccine table entry */
...
/* reads file given at file_ptr and updates the structures */
int update_structures(FILE* file_ptr, HashT* VaccineTable, HashT* citizens, HashT* countries, int bloom_size, HashT* updated); 