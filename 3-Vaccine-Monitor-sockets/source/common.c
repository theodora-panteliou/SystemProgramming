...
void send(int writefd, char* message, int message_size, int buffer_size, char* buff){
    ...
    while (message_size - bytes_written>0){ /* if there are bytes left */
...
void receive_message(int readfd, char** message, int* size, int buffer_size){
    ...
    bytes_read = 0;
    while (sizeof(int) - bytes_read> buffer_size){
    ...
    while (message_size-bytes_read>0){
        memset(buff, 0, buffer_size);
    ...
    // printf("bytes read %d mess size %d\n", bytes_read, message_size);
