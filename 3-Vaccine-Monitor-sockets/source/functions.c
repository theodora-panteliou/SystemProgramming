...
pthread_mutex_t strtok_mtx = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t vaccinetable_mtx = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t citizen_mtx = PTHREAD_MUTEX_INITIALIZER;
...
int input_read(char*  line, char** command, char** parameter, int array_size){
    ...
    pthread_mutex_unlock(&strtok_mtx);
    return par_count;
}
...
int update_structures(FILE* file_ptr, HashT* VaccineTable, HashT* citizens, HashT* countries, int bloom_size, HashT* updated){ /* call with updated = NULL the first time that VaccineTable is empty */    
    ...
    while (getline(&line, &linesize, file_ptr) != -1) {
        ...
        // if (( (temp_rec = HashT_get(citizens, parameter[0])) != NULL) && 
        ...
        if (strcmp(parameter[6], "NO") == 0 ){
            ...
            // virus_entry = HashT_get(VaccineTable, parameter[5]);
            // pthread_mutex_unlock(&vaccinetable_mtx);
            // if ((virus_entry = HashT_get(VaccineTable, parameter[5])) != NULL && (skiplist_search(virus_entry->vaccinated_persons, parameter[0]) != NULL)){ /*citizen already registered as vaccinated*/
            //     printf("ERROR IN RECORD "); //<incosistent answer: citizen registered as vaccinated>
            //     input_print(parameter);
                
            // } 
            // if ( (virus_entry!=NULL && skiplist_search(virus_entry->non_vaccinated_persons, parameter[0]) == NULL) || virus_entry==NULL){ /* insert case (if citizen is not already registered as non vaccinated) */
            pthread_mutex_lock(&citizen_mtx);
            temp_rec = HashT_get(citizens, parameter[0]);
            ...
            pthread_mutex_unlock(&citizen_mtx);
            pthread_mutex_lock(&vaccinetable_mtx);
            ...
            if (updated_entry!=NULL) {
                ...
                HashT_insert(updated, updated_entry->name, updated_entry); /* insert to updatted if updated!=NULL */     
            ...
            // if ( virus_entry != NULL && ((VirusYES_rec = skiplist_search(virus_entry->vaccinated_persons, parameter[0])) != NULL) ){ /*citizen already vaccinated: error case*/
